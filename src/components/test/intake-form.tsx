...
export function IntakeForm({ domaines }: Props) {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    // Tout est valide : on ouvre le popup d'avertissement avant de démarrer.
    setDonneesValidees(parsed.data);
  }

  function demarrerTest() {
    if (!donneesValidees) return;
  ...
  return (
    <Card>
      ...
      {/* Popup d'avertissement : visible une fois le formulaire validé,
          avant que le test ne soit officiellement démarré. */}
      <Dialog
        open={donneesValidees !== null}
        onOpenChange={(open) => { if (!open) setDonneesValidees(null); }}
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogTitle className="text-center">Avant de commencer</DialogTitle>
            <DialogDescription className="text-center">
              Il n&apos;est pas possible de reprendre ce test une fois entamé.
          ...
          <DialogFooter className="sm:justify-center">
            <Button
              type="button"
              variant="outline"
              onClick={() => setDonneesValidees(null)}
              disabled={isPending}
            >
              Pas maintenant
            </Button>
            <Button
              ...
              disabled={isPending}
            >
              {isPending ? t.commun.chargement : "J'ai du temps, commencer"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
